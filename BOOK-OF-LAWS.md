...
**Struck Jul 20, 2026 — Law 56, print path.** The customer document price path was unfenced (`epp-roundtrip` proved save/reload only, never the render) — the gap that let the §10.2 defect ship. Now fenced by `quote-document-fence.test.mjs`, mutation-tested. Remaining edge: `buildQuoteData` is a closure over component state and cannot be imported, so its call sites are pinned structurally rather than executed; extracting the document mapping into `lib/` would close that edge.
