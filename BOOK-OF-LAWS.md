# TPM BOOK OF LAWS — v0.2.4

**Status: v0.2.4 — amended Aug 7, 2026 (Law 83 — The Bid Is Frozen; the Contract Accretes — appended; 83 laws, gaveled by Tom). Built on v0.2.3 — amended Jul 30, 2026 (Law 82 — PMZ Informs, the Owner Decides — appended; 82 laws, gaveled by Tom). Built on v0.2.2 — amended Jul 20, 2026. Amendments: (1) **Law 19** annotated — customer-facing documents carry NO lifecycle vocabulary; canonical words govern internal surfaces only. (2) **Law 56** annotated — print-path divergence confirmed by owner walk, fixed and fenced; the print path is no longer unfenced. (3) **Law 65** corrected — project-pricer measures **0** TypeScript errors; the "~50 pre-existing" claim was stale. (4) **Laws 79–81 appended** — One Recovery Law, Idle Resource Model, Teaching Law. (5) `BACKEND-HANDOFF.md` §10.1 / §10.2 marked RESOLVED. Built on v0.2.1 (Jul 18, 2026) and v0.2 FINAL — Tom's red-pen Jul 17, 2026. All merge conflicts gaveled closed (see RESOLVED). No open conflicts.**
**Numbering rule: existing laws are NEVER renumbered — citations depend on stable numbers. New laws append at the end with their chapter noted.**
...
50. **LEM Gate** — **AMENDED Jul 25, 2026 (gaveled).** A **typed zero** LEM quantity/hours no longer hard-blocks the transition to Accepted — it **confirms and carries**: the owner is shown a confirmation that **names each zero field** (line · category · item) and its **consequence** — that item carries **$0 cost** into the accepted quote and the foreman's variance baseline — and may proceed if he means it. A zero is a true answer with the owner as its named source (Law 7); the software must not refuse a true answer. The confirmation is **persisted on the quote** (`zeroConfirmation`) so a confirmed zero is distinguishable from an unconfirmed one on the record later. Confirming is **permission to proceed, not evidence of cost** — it never sets a cost basis, so `hasCostBasis` still fails and Earned Green stays unreachable this way. **Genuinely incomplete detail still blocks**: a **blank (missing)** quantity/hours, or a line with **no LEM entries at all**, is the *absence* of an answer, not a zero — the transition is refused until a number is entered (a confirmable blank would reopen Earned Green from the other side). ~~SUPERSEDED: "No transition to Accepted while lines carry zero quantities" — a zero hard-blocked, indistinguishable from a blank.~~ *(Build F; amended/gaveled Jul 25 2026)* — `lem-gate-fence.test.mjs`
51. **Overhead Store & Math** — Chart persists under `pmz_overhead_chart`; Overhead = Σ chart.items.amount; overheadPercentOfRevenue = overhead ÷ revenue = the company-scope handoff value. **AMENDED Jul 23, 2026 (gaveled):** the chart is now a **LEDGER only** — it no longer feeds per-job overhead allocation; per-job overhead derives from the Work Types planning store (see Law 55). *(Build F; ledger role clarified Jul 23 2026)* — `BUILD-F-SCOPING.md:75,84-86`
52. **Countdown Law** — overheadRemaining = max(0, overhead − projectedGrossProfit), counting to zero; uses the page's projected figures, not PMZ job actuals. The `overhead` here is the **company-scope ledger total** (Law 51), NOT the per-job planned rate (Law 55, amended). *(Build F; scope clarified Jul 23 2026)* — `BUILD-F-SCOPING.md:89-95`
...
55. **Planned-Rate Map Math** — **AMENDED Jul 23, 2026 (gaveled).** A job's overhead = job revenue × its work type's **PLANNED OVERHEAD RATE** = that work type's allocated overhead ÷ its target revenue (from the Work Types planning store: `pool × weight ÷ Σ(targetRevenue × weight)`). When no planned rate exists (no pool, no target revenue for the work type, or no allocation base), the overhead rung renders the empty state **"Set your Planned Annual Overhead in Work Types to allocate overhead"** — never the old ratio, never a fake $0, never a blowup — and net is unknown too; the rung carries the source label **"at your planned overhead rate (X%)"**. rev=0 resolves all percentages to 0 (no divide-by-zero, no NaN). The Overhead & Profit chart is a **ledger only** and no longer feeds job allocation. ~~**SUPERSEDED** (was: overhead = round(rev × Σitems/monthlyRevenue), the chart÷invoiced-revenue ratio) — its tiny-denominator blowups on a thin invoiced base were the defect that forced this gavel.~~ Fence rewritten to pin the new law; mutation-tested — **wrong-rate FAILS and revert-to-old-ratio FAILS**. *(Jul 17; amended/gaveled Jul 23 2026)* — `pipeline-fence.test.mjs`
56. **One Price Path** — Worksheet display and save both go through `lib/epp-line`; a line's price and the quote total can never diverge; manual unitPrice, priceOverridden, and all per-line LEM detail survive save → reload intact; scope-only lines stay clean; persisted totalRevenue equals the worksheet total. **The customer document prints the QUOTED price, never the Golden Formula recommendation** — the recommendation is owner-facing coaching only (the worksheet's "Recommended @ %" line). **Printed === persisted, with no presentation-only rounding anywhere on customer paper**; amounts print to the cent. Round-dollar quoting, if ever wanted, rounds the SAVED price at entry — parked for a future gavel, never a print-time transform. **PRINT-PATH DIVERGENCE OF RECORD:** confirmed by owner walk Jul 20, 2026 — the preview/PDF printed cost-derived recommended prices instead of the persisted quote; a no-cost quote printed **$0** against a $75,495.28 worksheet, and a $37,880.00 quote printed **$35,406** (exactly its "Recommended @ 20%" figure). Root cause: `buildQuoteData` built line totals from `customerUnitPrice` (Golden Formula) plus whole-dollar `roundToQuote`. Fixed `0196a9a`. **The print path is no longer unfenced** — `d246303` asserts printed total === persisted totalRevenue over manual, overridden, cents, fractional, no-cost, scope-only and empty quotes, pins both walk cases as regressions, and structurally bars `customerUnitPrice`/`roundToQuote` from `buildQuoteData`; both halves mutation-tested (proven to FAIL when the defect is reintroduced). Verified on production Jul 20. *(Standing; print path confirmed/fixed/fenced Jul 20)* — `epp-roundtrip.test.mjs:2-6,19-20,38-41,63-74`; `quote-document-fence.test.mjs`
...
65. **Zero TypeScript Errors** — Enforced on every commit, no exceptions; never raise a file's existing error count. **CORRECTED Jul 20, 2026:** the long-standing "project-pricer carries ~50 pre-existing errors" claim is **stale** — `npm run typecheck` (`tsc --noEmit`) measures **0 errors** repo-wide, project-pricer included. A clean typecheck is the baseline; any error introduced is the builder's own. `CLAUDE.md:26` still carries the stale "~50" figure and owes the same correction at its next touch. *(Standing; measured and corrected Jul 20)* — `CLAUDE.md:26`
...
## APPENDED LAWS (chapter noted per law — never renumbered into their chapters)

> **⚠️ RULED Jul 20, 2026 — NOT YET BUILT (laws 79–80).** These are gaveled canon and binding on design, but no code implements them yet. Law 81 is STANDING and effective now.

*(Law 78, Edition Citation Law, was appended Jul 18 and sits in Ch VII above — left in place, not renumbered.)*

79. **One Recovery Law** *(Ch III — Pricing)* — Equipment rates are **pure ownership cost ÷ total available hours**. Available hours = realistic season hours, with the **ANNUAL** cost in the numerator. Idle time recovers **ONLY** as a named Indirect COGS line — never by shrinking the hour divisor. One recovery method, never both: an idle-inflated rate *and* an idle line double-charges the same cost. The existing rate builder converts to pure rates at the Idle Iron segment; **a rate audit is step one**. *(Jul 20, gaveled)* 🔓 **NOT YET BUILT**

80. **Idle Resource Model** *(Ch III — Pricing)* — Rung 3 launches with exactly four named lines: **Idle Equipment · Idle Labor · Small Tools & Consumables · Warranty Reserve**. Predictable accruals (payroll taxes, workers' comp, PTO) load the **labor rate**; event-driven idle (weather, standby, training, onboarding) hits the **Idle Labor line** via one-tap tagging. Supervision splits by a **set-once percentage** — job share → indirect, management share → overhead — **never hour-by-hour**. The yard is overhead; **machine-specific storage is that machine's ownership cost**. *(Jul 20, gaveled)* 🔓 **NOT YET BUILT**

81. **Teaching Law** *(Ch IV — UI / Poka-Yoke; **STANDING, effective now**)* — The product asks **life questions, never accounting questions**; the theory lives **one tap behind the number**. Revelation first, explanation on tap. **Defaults over decisions.** Sorting resolves via one binary: *"would this cost exist in a month with zero jobs?"* — yes → overhead, no → job cost. *(Jul 20, gaveled)*

82. **PMZ Informs, the Owner Decides** *(Ch V — Audience; **STANDING, effective now**)* — PMZ surfaces the numbers: margin position, overhead coverage, where the year stands. It never fires triggers, auto-adjusts margins, or tells the owner what to do with that information. Post-overhead margin strategy — push harder, hold steady, or take a strategic job at a tightened margin — is a judgment call made by the owner or CFO, outside the software. The software's job is to make sure that judgment is informed, recorded, and attributable. *(Origin: gaveled by Tom, 2026-07-30)*

83. **The Bid Is Frozen; the Contract Accretes** *(Ch III — Pricing; sits beside Law 56; **STANDING, effective now**)* — The entered bid is frozen at save and is never edited (Law 56 stands). Growth after acceptance happens only by addendum: a change order, priced from the frozen bid's own margin, stamped with its origin, standing beside the bid — never inside it. The contract total is the frozen bid plus its released change orders, and that is what gets invoiced. The bid tells you what was promised; the change orders tell you what was added; the contract total tells you what is owed. Nothing already written is ever rewritten. *(Origin: gaveled by Tom, 2026-08-07, upon the first change order reaching a customer document)*
...
Scope note: process laws (Ch VII) are fenced by discipline, not tests, and are excluded. Listed = product/data laws a test COULD fence but none does, per the sweep of `pipeline-fence.test.mjs`, `superuser-status.test.mjs`, `epp-roundtrip.test.mjs`, `quote-document-fence.test.mjs`.

**Struck Jul 20, 2026 — Law 56, print path.** The customer document price path was unfenced (`epp-roundtrip` proved save/reload only, never the render) — the gap that let the §10.2 defect ship. Now fenced by `quote-document-fence.test.mjs`, mutation-tested.

**Edge closed Jul 22, 2026 — Law 56, executed not pinned.** The remaining edge above (`buildQuoteData` was a closure over component state, so its call sites were pinned by source text rather than executed) is gone. The document mapping now lives in `lib/quote-document.ts` as `buildQuoteDocument`, with component state passed as explicit deps and an injectable clock for determinism. `quote-document-fence.test.mjs` calls the REAL function; re-mutation-tested at extraction — a cost-derived unit price and presentation rounding each fail the fence. **Law 56 is fenced by execution, end to end.**
...
- **Law 19, Status Lifecycle Language** — header vocabulary FENCED as of `86f7066` (phase headers renamed to canonical status groupings; vocabulary fence updated and green) — struck from this list for header vocabulary. **Customer-document edge closed Jul 20** by removal rather than by fence: those documents now carry no lifecycle vocabulary at all (`c8c05c7`), so there is no label mapping left to fence there. Remaining unfenced edge: status-label mapping on **internal** non-pipeline surfaces.
